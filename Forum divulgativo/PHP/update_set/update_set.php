        ...
        Class Update extends Banco
        {
            ...
            protected function __construct($sql_parametros,$parametros_banco_de_dados,$dados_parametros)
            {
                ...
                if($this->connecting_exporting)
                {
                    try
                    {
                        $this->stmt = $this->connecting_exporting->prepare($this->sql_propriedades['update']);
                ...
                {
                    $this->status = 3;
                    // "Banco de Dados Não foi Exportado";
                }
            }
            public function __getUpdate()
            {
                return $this->status;
            }
            public static function UpdateGet($sql_parametros,$parametros_banco_de_dados,$dados_parametros)
            {
