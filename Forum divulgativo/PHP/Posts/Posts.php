<?php
    ...
    if(isset($_SESSION['Administrador']['status']) && !empty($_SESSION['Administrador']['status']))
    {
        ...
        <?php
            ...
            $image = Image($_FILES,$_POST['valor_da_foto']);
            $condicao = null;
            if(is_array($image))
            {
                $Pasta = "../../Img-Posted-For-Adm/";
                if(move_uploaded_file($image['nome_temporario'],$Pasta.$image['nome']))
                {
                    $colunas = ['titulo' => $_POST['title-post'], 'image' => $image['nome'],'categoria' => $_POST['category']];
                    $null = [];
                    $tabela = CreateTables::iniciar(CreateFunctions(),DatabaseConneting_Params())->__getCreateTablesResult()[2];
                    $insert = Insert::AutoIntance(MySQL($colunas,$tabela,$null),DatabaseConneting_Params(),$colunas)->__getResult();
                    $condicao = $insert;
                }
                else
                {
                    $condicao = 3;
                    // "Erro na hora de deslocar imagem para servidor local";
                }
            }
            else
            {
        ...
        ?>
        <input type="hidden" id="Identify" value="<?php echo $condicao;?>">
        </body>
        <script src="../../JavaScript/PopUp-Encode-Posts.js"></script>
        </html>
        <?php
        "
        ";
